...
module Make (Y : Json_impl.S) : S with type elt = Y.t = struct
  ...
  module Error = struct
    ...
    let rec show = function
      | Field (field_name, t) ->
        Printf.sprintf "Error in field %s: %s" field_name (show t)
      | Index (idx, t) -> Printf.sprintf "Error at index %i: %s" idx (show t)
      | One_of errors -> List.fold_left (fun acc t -> acc ^ show t) "" errors
      | Failure (msg, elt) -> Printf.sprintf "Failure %s at %s" msg (Y.show elt)
    ;;
